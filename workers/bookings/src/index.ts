 ...
 */
import { curacaoBookingCore } from "../../../shared/destinations/curacao";
...
async function handlePreviewRequest(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message:
          "Live Stripe is not enabled for this Curacao Worker. Charge-then-refund still requires an explicit production configuration. No payment was taken.",
      },
      503,
    );
  }
  ...
  const product = findCuracaoBookingProduct(body.productId);
  ...
  const reference = createBookingReference(curacaoBookingCore);
  ...
  const destinationBrand = destinationBrandFromCore(curacaoBookingCore);
